(function() {
  ...
  var defaults = {
    ...
    contentSelector: '.slideview-slide', 
    
    // TODO:
    nextButton: '.slideview-next',
    prevButton: '.slideview-prev',
    pagination: '.slideview-pagination',
    paginationItem: function(index, active) {
      return '<span class="' + className +" " + (active ? 'active' : '') + '"></span>';
    }
  ...
  var pluginClass = function SlideView(element, options) {
    ...
    function updateControls() {
      ...
      var nextButton = getControlElement('nextButton');
      var prevButton = getControlElement('prevButton');
      console.log("nextButton", nextButton);
    }
    
    function getControlElement(name) {
      var value = options[name]; 
      var control = value ? $(value)[0] : null;
      var cached = optionsCache[name];
      if (cached && (cached !== control || cached.outerHTML !== value)) {
      ...
      //$element.bind(touchEndEvent, function(event) {
        ...
        if (touchCurrentPos != null) {   
          ...
          if (o >= 1 && dx != 0) {
            ...
            // slideView.slideTo(e, {transition: 'swipe', });
    ...
    function loadContent(item, callback) {
      ...
      $.ajax({
        ...
        $(htmlElements).each(function() {
          var importNode = null;
          var $this = $(this);
          if ($this.is(options.contentSelector)) {
            importNode = this; 
          } else {
            importNode = $this.find(options.contentSelector)[0];
          }
          if (importNode) {
        ...
        if (elem) {
          ...
          console.warn('content element not found - selector: ' + options.contentSelector);
        }
